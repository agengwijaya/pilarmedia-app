              ...
              <tbody>
                @foreach ($penjualan_sales as $item)
                  ...
                  @foreach ($produk as $value)
                    @php
                      $sales = $item->sales->where('products_id', $value->id)->sum('sales_amount');
                    @endphp
                    <tr>
                      <th>{{ $item->nama }}</th>
                      <td>{{ $value->nama }}</td>
      ...
      <div class="col-lg-6">
        ...
        <div class="row">
          <div class="col-4">
            <div class="mb-3 mt-2">
              <label for="exampleInputEmail1" class="form-label">Tanggal Awal</label>
              <input type="date" class="form-control" id="tgl_awal" name="tgl_awal" value="{{ date('Y-m-01') }}">
            </div>
          </div>
          <div class="col-4">
            <div class="mb-3 mt-2">
    ...
    Highcharts.chart('container', {
      ...
      series: [
        <?php foreach ($penjualan_sales as $value): ?> {
          ...
          data: [
            <?php $harga = 0; ?>
            <?php foreach ($value->sales as $val): ?>
            <?php $harga += $val->sales_amount ?>
            <?php endforeach; ?>
            {{ $harga }},
          ]
        },
